        ...
        <div class="container">
          ...
          <div class="text-center my-6" v-if="!loading && data.length > 0">
            ...
            <div class="btn-group">
              ...
              <b-dropdown text="" style="height: 36px; opacity: .95" variant="primary">
                <b-dropdown-item  @click="_createExcel('PT. Indofood CBP Sukses Makmur Tbk Harian')">
                  PT. Indofood CBP Sukses Makmur Tbk &nbsp
                  <span class="badge w-100 bg-orange"> Harian </span>
                </b-dropdown-item>
                <b-dropdown-item  @click="_createExcel('PT. Indofood CBP Sukses Makmur Tbk Mingguan')">
                  PT. Indofood CBP Sukses Makmur Tbk &nbsp
                  <span class="badge w-100 bg-info"> Mingguan </span>
                </b-dropdown-item>
              </b-dropdown>
            </div>
...
export default {
  ...
  methods: {
      ...
      _createExcel(params){
        switch(params) {
          case "PT. Indofood CBP Sukses Makmur Tbk Harian":
            return this._createExcelIndofood('harian')
            break;
          case "PT. Indofood CBP Sukses Makmur Tbk Mingguan":
            return this._createExcelIndofood('mingguan')
            break;
          default:
            return this._createExcelDefault()
        }
      },
      ...
      _createExcelIndofood(params){
        var dataExcel = [];
        if(params=='mingguan'){
          for(let i = 0; i < this.data.length; i++){
            let d           = this.data[i];
            let tipe_toko_2 = d.tipe_toko_2 ? d.tipe_toko_2.toUpperCase() : ''
            let pasar       = tipe_toko_2 == 'PASAR' || tipe_toko_2 == 'R2' || tipe_toko_2 == 'RO2' ? 'Pasar' : 'Non Pasar'
            ...
            dataExcel.push({
              ...
              'distrik': 'Bali',
              'lokasi_pasar' : pasar,
              'nama_pasar' : pasar == 'Pasar' ? d.alamat_toko : '',
              'itemcode': d.item_code,
              'scode': d.pcs_code == '' ? d.kode_barang : d.pcs_code,
              'groupcode': d.nama_segmen,
              'brand': d.nama_brand,
              'slsmname': d.nama_salesman,
              'team': d.nama_tim,
              'depo': d.nama_depo,
              'in_dus' : { v: (parseFloat(d.qty_dus)+(parseFloat(d.qty_pcs)/parseFloat(d.isi))), t: "n"},
              'tahun': d.tahun,
              'bulan': d.bulan,
              'hari': d.hari,
              'proposal': d.promo,
        ...
        this.createExcel({data: dataExcel, filename: 'laporan-stt-format-indofood-'+this.dataFilter.start_date+' - '+this.dataFilter.end_date}, false)
      }
  },
