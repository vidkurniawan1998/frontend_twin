                  ...
                  <table class="table table-vcenter table-hover card-table table-striped">
                    ...
                    <tbody>
                      <tr v-for="(row, i) in data" :key="i" :class="i == 0 && paginate.currentPage == 1 ? animate : ''">
                        ...
                        <td nowrap class="click-able text-primary" @click="_detail(row)">
                          PO: {{row.id}} <br>
                          {{row.no_invoice}}
                        </td>
                        ...
                        <td nowrap>{{row.nama_tim}} - {{row.salesman[0].nama_salesman.slice(0,10)}}</td>
                        <td nowrap>{{dateFormat(row.tanggal)}}</td>
                        <td nowrap>{{row.delivered_at != '' ? dFormat(row.delivered_at, 'dmY') : '-'}}</td>
                        ...
                        <td nowrap style="vertical-align : center">
                          <ul style="list-style-type:none; margin-top:10px">
                            <li v-if="row.created_at != ''"> <button class="btn btn-sm btn-warning">{{row.created_at}}</button></li>
                            <li v-if="row.approved_at != ''"> <button class="btn btn-sm btn-success">{{row.approved_at}}</button></li>
                            <li v-if="row.delivered_at != ''"> <button class="btn btn-sm btn-info">{{row.delivered_at}}</button></li>
                          </ul>
                        </td>
                        <td class="p-0">
