          ...
          <form @submit.prevent="submit">
            <div class="modal-body default">
              ...
              <div class="form-group">
                <label for="" class="form-label">Nama Segmen</label>
                <input type="text" class="form-control" v-model="data.nama_segmen" required placeholder="Nama segmen">
              </div>

              <div class="form-group">
                <label for="" class="form-label">Nama Brand</label>
                <selectize v-model="data.id_brand" data-placeholder="Pilih brand" required> 
                  <option :value="dt.id" v-for="(dt, i) in brands" :key="i"> {{ dt.nama_brand }} </option>
                </selectize>
              </div>
    ...
    "config.view": function(view) {
      if (view) {
        ...
        setTimeout(() => {
          ...
          if(this.brands.length == 0) this._getBrand()
        }, 100);
      } else {
        this.modal.open = view;
