<template>
  <div class="antialiased">
    ...
    <div class="page">
      ...
      <Header @change="_search" hint="Ketik nama gudang"/>
                  ...
                  <table class="table table-vcenter table-hover card-table table-striped">
                    ...
                    <tbody>
                      <tr v-for="(row, i) in data" :key="i">
                        ...
                        <td>
                          <span class="badge bg-blue-lt mr-2">{{row.nama_depo}}</span>
                        </td>
...
import FormGudang from "./modals/form-gudang.vue";

export default {
  name: "Gudang",
  components: { FormGudang },
  ...
  methods: {
      ...
      _get(params){
        ...
        this.axios.get('gudang?per_page='+this.paginate.perPage+'&page='+this.paginate.currentPage+'&keyword='+this.dataFilter.keyword).then(res => {
          this.loading = false;
      ...
      _add(){
        this.form.config = {view: true, title: 'Tambah Gudang'}
        this.form.data = null
        this.form.action = (formData) => {

          this.axios.post("gudang", formData).then(() => {
      ...
      _edit(data){
        this.form.config = {view: true, title: 'Edit Gudang'}
        this.form.data = data
        this.form.action = (formData) => {

          this.axios.put("gudang/"+data.id, formData).then(() => {
      ...
      _delete(id){
        this.confirm.id = id
        this.confirm.title = 'Yakin ingin menghapus data gudang ini?'
        this.confirm.action = () => {

          this.axios.delete('gudang/'+id).then( () => {
