<template>
  <div class="antialiased">
    ...
    <div class="page">
      ...
      <Header @change="_search" hint="Ketik nama principal"/>
                  ...
                  <table class="table table-vcenter table-hover card-table table-striped table-control">
                    ...
                    <tbody>
                      <tr v-for="(row, i) in filter" :key="i" >
                        ...
                        <td>
                          <span class="badge bg-blue-lt mr-2"> {{row.perusahaan.kode_perusahaan}} </span>
                        </td>
                        ...
                        <td class="p-0 pos-relative">
                          <div class="btn-group option center">
                            ...
                            <button class="btn btn-white btn-pill text-danger" @click="_delete(row.id)" v-html="ic('trash')"></button>
                          </div>
                        </td>
                      </tr>
                    </tbody>
...
import FormPrincipal from "./modals/form-principal.vue";

export default {
  name: "Principal",
  components: { FormPrincipal },
  ...
  methods: {
      ...
      _get(params){
        ...
        this.axios.get('principal?per_page='+this.paginate.perPage+'&page='+this.paginate.currentPage+'&keyword='+this.dataFilter.keyword).then(res => {
          this.loading = false;
      ...
      _add(data){
        this.form.config = {view: true, title: 'Tambah Principal'}
        this.form.data = null
        this.form.action = (formData) => {

          this.axios.post("principal", formData).then(() => {
      ...
      _edit(data){
        this.form.config = {view: true, title: 'Edit Principal'}
        this.form.data = data
        this.form.action = (formData) => {

          this.axios.put("principal/"+data.id, formData).then(() => {
            this._get({silent: true})
      ...
      _delete(id){
        this.confirm.id = id
        this.confirm.title = 'Yakin ingin menghapus data principal ini?'
        this.confirm.action = () => {

          this.axios.delete('principal/'+id).then( () => {
      ...
      _search(keyword){
        ...
        //   if(this.data[i].nama_principal.toLowerCase().includes(k)){
