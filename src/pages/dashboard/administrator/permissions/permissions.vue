<template>
  <div class="antialiased">
    ...
    <div class="page">
      ...
      <Header @change="_search" hint="Ketik nama permission"/>
  ...
  <script>
    ...
    import FormPermissions from "./modals/form-permissions.vue";

    export default {
      name: "Permission",
      components: { FormPermissions },
      ...
      methods: {
        ...
        _get(){
          ...
          let url = 'permission?per_page='+this.paginate.perPage+'&page='+this.paginate.currentPage+'&keyword='+this.dataFilter.keyword;
        ...
        _add(data){
          this.form.config = {view: true, title: 'Tambah Permission'}
          this.form.data = data
          this.form.action = (formData) => {
            formData.name = this.ucwords(formData.name)
            
            this.axios.post('permission', formData).then(() => {
              this._get()
        ...
        _edit(data){
          this.form.config = {view: true, title: 'Edit Permission'}
          this.form.data = data
          this.form.action = (formData) => {
            formData.name = this.ucwords(formData.name)
            
            this.axios.put('permission/'+data.id, formData).then(() => {
        ...
        _delete(id){
          this.confirm.id = id
          this.confirm.title = 'Yakin ingin menghapus permission ini?'
          this.confirm.action = () => {
            
            this.axios.delete('permission/'+id).then(() => {
